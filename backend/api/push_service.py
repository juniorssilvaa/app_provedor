        ...
        try:
            ...
            if response.responses:
                for idx, resp in enumerate(response.responses):
                    token = batch_tokens[idx] if idx < len(batch_tokens) else None
                    if resp.success:
                        # Marca dispositivo como confirmado
                        try:
                            Device.objects.filter(push_token=token).update(push_confirmed=True, push_confirmed_at=timezone.now())
                        except Exception:
                            pass
                    else:
                        error_code = 'unknown'
