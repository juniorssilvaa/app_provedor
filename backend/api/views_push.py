...
def register_device(request):
    ...
    try:
        ...
        # Extração robusta de modelo e versão do SO a partir de múltiplas chaves
        def _first_non_empty(*keys):
            for k in keys:
                ...
                if isinstance(device_info, dict):
                    v2 = device_info.get(k)
                    if v2:
                        return v2
            return None

        device_model = _first_non_empty('model', 'device_model', 'modelo', 'model_name') or 'Unknown'
        os_version = (
            _first_non_empty('os_version', 'android_version', 'version', 'versao', 'versao_android')
            or 'Unknown'
        )
        ...
        if existing_device:
            ...
            # Atualiza modelo e versão quando vierem no payload
...
# --------------------------------------------------
# 5) WEBHOOK GENÉRICO (POST /webhooks/push/)
# --------------------------------------------------
