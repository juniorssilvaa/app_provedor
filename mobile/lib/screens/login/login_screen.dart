    ...
    try {
      ...
      try {
        ...
        if (contratos.isNotEmpty) {
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            // Lógica atualizada: Verifica se é SUSPENSO ou CANCELADO (status 3 costuma ser cancelado)
                            // Se o texto contiver "CANCELADO", também considera
           ...
           clientData = {
             'nome': contratos.first['razaoSocial'] ?? contratos.first['razaosocial'] ?? 'Cliente Desconhecido',
           ...
           allMappedContracts.insert(0, selectedContract);
            ...
            // Se contratos estiver vazio mas result não for null, verificar se tem mensagem de erro
            if (result != null) {
               final msg = result['msg'] ?? result['message'] ?? result['error'];
               if (msg != null) throw Exception(msg);
            }
            throw Exception('Nenhum contrato ativo encontrado para este CPF.');
         }
      } catch (e) {
      ...
      final String userName = clientData['nome'] ?? clientData['razao_social'] ?? 'Cliente Desconhecido';
